...
# Connect to database and load data for specific tickers
...
# Calculate 10-day Simple Moving Average (SMA) of Adj_Close
# min_periods=1 ensures that SMA is calculated even if fewer than 10 periods are available at the start
df['SMA_10_Day'] = df.groupby('Ticker')['Adj_Close'].rolling(window=10, min_periods=1).mean().reset_index(level=0, drop=True)

# Calculate 5-day Simple Moving Average (SMA) of Adj_Close
# min_periods=1 ensures that SMA is calculated even if fewer than 5 periods are available at the start
